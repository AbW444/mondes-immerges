...
`),(t?t.querySelectorAll("button"):[]).forEach(i=>{i.style.cssText=`
    ...
    `,document.head.appendChild(t)}function Me(){if(b.initialized||(b.initialized=!0,!me()))return;Ee();let r=0;const e=setInterval(()=>{r+=12.5,r>=100&&(clearInterval(e),setTimeout(()=>{ye()},300))},200)}function Ce(){if(b.cleanupDone)return;b.cleanupDone=!0;const r=j();r&&r.destroy&&r.destroy()}document.addEventListener("DOMContentLoaded",()=>{b.domReady||(b.domReady=!0,ge(),setTimeout(()=>{Me()},100))});window.addEventListener("error",r=>{r.target!==window&&(r.target.tagName==="IMG"||r.target.tagName==="SCRIPT")||console.warn("[MondesImmerges]",r.error||r.message)},{once:!1,capture:!0});window.addEventListener("unhandledrejection",r=>{console.warn("[MondesImmerges]",r.reason)},{once:!1});window.addEventListener("beforeunload",Ce,{once:!0});window.addEventListener("pageshow",r=>{(r.persisted||window.performance&&window.performance.navigation.type===2)&&window.location.reload()});
