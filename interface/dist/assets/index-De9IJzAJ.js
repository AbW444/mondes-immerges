...
`),(t?t.querySelectorAll("button"):[]).forEach(o=>{o.style.cssText=`
    ...
    `,document.head.appendChild(e);const t=document.getElementById("welcome-screen");t&&(t.style.display="none",t.classList.add("hidden"));const i=document.getElementById("main-container");i&&(i.classList.add("hidden"),i.style.opacity="0");let o=0;const s=8,n=()=>{o++;const r=Math.round(o/s*100);console.log(`📦 Ressource ${o}/${s} chargée (${r}%)`);const d=document.querySelector(".progress-bar"),u=document.querySelector(".progress-text");d&&(d.style.width=`${r}%`),u&&(u.textContent=`${r}%`),o>=s&&(console.log("✅ Toutes les ressources chargées - Démarrage de l'application"),xe())};["Configuration système","Textures globe","Vidéos des zones","Données géographiques","Interfaces utilisateur","Modèles 3D","Données scientifiques","Effets visuels"].forEach((r,d)=>{const u=150+Math.random()*250;setTimeout(()=>{console.log(`📦 Ressource chargée: ${r}`),n()},u*(d+1))})}function xe(){console.log("🚀 Démarrage de l'application...");try{ge();const l=fe();if(!l)throw new Error("Impossible de récupérer l'instance de l'application");console.log("✅ Application initialisée avec succès"),setTimeout(()=>{we(l)},300)}catch(l){console.error("❌ Erreur lors de l'initialisation de l'application:",l),ve(l)}}function ve(l){const e=document.createElement("div");e.style.cssText=`
        position: fixed;
        top: 50%;
        left: 50%;
        transform: translate(-50%, -50%);
    ...
    `,document.body.appendChild(e)}function we(l){console.log("🎬 Démarrage de la séquence d'initialisation...");const e=document.getElementById("loading-screen");e?(e.style.opacity="0",e.style.transform="scale(1.1)",setTimeout(()=>{e.classList.add("hidden");const t=document.getElementById("main-container");t&&(t.classList.remove("hidden"),t.style.opacity="1"),console.log("🎭 Conteneur principal rendu visible"),l.startupSequence&&typeof l.startupSequence=="function"?(console.log("🚀 Lancement de la séquence de démarrage..."),l.startupSequence(),setTimeout(()=>{console.log("🌊 Démarrage de l'exploration..."),l.startExploration(!0)},5e3)):(console.log("⚠️  Méthode startupSequence non trouvée, démarrage direct"),l.startExploration(!0))},500)):(console.log("⚠️  Écran de chargement non trouvé, démarrage direct"),l.startExploration(!0))}function Ee(){const l=document.getElementById("loading-screen");if(!l){console.warn("⚠️  Écran de chargement non trouvé");return}if(l.querySelector(".progress-container")){console.log("🔄 Écran de chargement déjà amélioré");return}console.log("✨ Amélioration de l'écran de chargement...");const e=l.querySelector(".spinner");e&&e.remove();try{V.register();const s=document.createElement("l-jelly");s.setAttribute("size","120"),s.setAttribute("speed","0.9"),s.setAttribute("color","#ffcc00"),s.style.cssText=`
            margin-bottom: 30px;
