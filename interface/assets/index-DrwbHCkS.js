...
`),(t?t.querySelectorAll("button"):[]).forEach(o=>{o.style.cssText=`
        ...
        `)}function ye(){const r=document.querySelector(".cursor"),e=document.querySelector(".cursor-follower");if(!r||!e)return;let t=0,i=0,o=0,s=0,n=null;const a=d=>{t=d.clientX,i=d.clientY};document.addEventListener("mousemove",a,{passive:!0});function c(){r.style.left=t+"px",r.style.top=i+"px",o+=(t-o)*.1,s+=(i-s)*.1,e.style.left=o+"px",e.style.top=s+"px",n=requestAnimationFrame(c)}n=requestAnimationFrame(c),window.addEventListener("beforeunload",()=>{n&&cancelAnimationFrame(n),document.removeEventListener("mousemove",a)}),console.log("✅ Curseur initialisé")}async function xe(){return new Promise(r=>{const e=document.querySelector("l-jelly"),t=document.getElementById("transition-video");if(!t){console.warn("⚠️ Vidéo de transition introuvable"),r();return}console.log("🎬 Lecture vidéo de transition en reverse..."),e&&(e.style.transition="opacity 0.3s ease",e.style.opacity="0",setTimeout(()=>e.remove(),300)),t.addEventListener("loadedmetadata",()=>{t.currentTime=t.duration,t.playbackRate=-1,t.style.opacity="1",t.play().catch(i=>{console.warn("⚠️ Erreur lecture vidéo transition:",i),r()})},{once:!0}),t.readyState>=1?(t.currentTime=t.duration,t.playbackRate=-1,t.style.opacity="1",t.play().catch(i=>{console.warn("⚠️ Erreur lecture vidéo transition:",i),r()})):t.load(),t.addEventListener("ended",()=>{console.log("✅ Vidéo de transition terminée"),t.style.opacity="0",setTimeout(()=>r(),300)},{once:!0}),setTimeout(()=>{console.warn("⚠️ Timeout vidéo transition"),r()},1e4)})}async function ve(){if(y.appStarted){console.warn("⚠️  Application déjà démarrée");return}console.log("🚀 Démarrage application..."),y.appStarted=!0;try{he();const r=ge();if(!r)throw new Error("Instance application non disponible");console.log("✅ Application initialisée"),we(),console.log("⏳ PRÉCHARGEMENT DES ASSETS EN COURS..."),r.globeManager&&r.globeManager.preloadAllAssets?(await r.globeManager.preloadAllAssets(),console.log("✅ TOUS LES ASSETS SONT PRÊTS")):console.warn("⚠️  preloadAllAssets non disponible, passage direct"),await xe(),setTimeout(()=>{r.startupSequence&&typeof r.startupSequence=="function"?(console.log("🎬 Séquence de démarrage..."),r.startupSequence(()=>{V(),setTimeout(()=>{console.log("🌊 Démarrage exploration..."),r.startExploration(!0),setTimeout(()=>{$(r)},200)},2500)})):(console.log("🌊 Démarrage direct..."),V(),setTimeout(()=>{r.startExploration(!0),setTimeout(()=>{$(r)},200)},2500))},300)}catch(r){console.error("❌ Erreur application:",r),Ee()}}function we(){const r=document.getElementById("main-container");if(!r)return;r.classList.remove("hidden"),r.style.opacity="0",r.style.transition="opacity 1s cubic-bezier(0.19, 1, 0.22, 1)";const e=document.getElementById("ui-controls"),t=document.querySelectorAll(".satellite-hud, .coordinates-display"),i=document.querySelector(".satellite-crosshair"),o=document.querySelector(".scanner-effect");e&&(e.style.opacity="0",e.style.transform="translateY(20px)",e.style.pointerEvents="none"),t.forEach(s=>{s.style.opacity="0"}),i&&(i.style.opacity="0"),o&&(o.style.opacity="0"),console.log("✅ Interface initialisée en mode clear")}function $(r){const e=document.getElementById("ui-controls"),t=document.querySelectorAll(".satellite-hud, .coordinates-display"),i=document.querySelector(".satellite-crosshair"),o=document.querySelectorAll(".hotspot-label"),s=document.querySelectorAll("svg"),n=document.querySelector(".scanner-effect");console.log("✨ Apparition des éléments UI + hotspots + connector lines"),r&&r.globeManager&&r.globeManager.showLabels(),e&&(e.style.pointerEvents="auto"),window.gsap?(e&&gsap.to(e,{opacity:1,y:0,duration:.8,ease:"power2.out"}),gsap.to([...t,i,n].filter(Boolean),{opacity:1,duration:.8,ease:"power2.out"}),gsap.to([...o,...s],{opacity:1,duration:.8,ease:"power2.out"})):(e&&(e.style.transition="all 0.8s ease",e.style.opacity="1",e.style.transform="translateY(0)"),t.forEach(a=>{a.style.transition="opacity 0.8s ease",a.style.opacity="1"}),i&&(i.style.transition="opacity 0.8s ease",i.style.opacity="1"),n&&(n.style.transition="opacity 0.8s ease",n.style.opacity="1"),o.forEach(a=>{a.style.transition="opacity 0.8s ease",a.style.opacity="1"}),s.forEach(a=>{a.style.transition="opacity 0.8s ease",a.style.opacity="1"}))}function V(){const r=document.getElementById("loading-screen"),e=document.getElementById("main-container");r&&(console.log("🌑 Début transition fond noir (100% → 0% sur 4s)"),console.log("🌍 Interface (globe) visible immédiatement derrière le fond noir"),e&&(e.style.opacity="1"),r.style.transition="opacity 2.5s cubic-bezier(0.19, 1, 0.22, 1)",r.style.opacity="0",setTimeout(()=>{r.style.display="none",r.classList.add("hidden"),console.log("✅ Écran de chargement complètement masqué")},2500))}function Ee(r){const e=document.getElementById("loading-screen");e&&(e.innerHTML=`
    ...
    `,document.head.appendChild(t)}function Ce(){if(y.initialized){console.warn("⚠️  Déjà initialisé");return}if(console.log("🌊 Initialisation Mondes Immergés..."),y.initialized=!0,!fe())return;Me();let r=0;const e=setInterval(()=>{r+=12.5,r>=100&&(clearInterval(e),console.log("✅ Chargement terminé"),setTimeout(()=>{ve()},300))},200)}function Se(){y.cleanupDone||(console.log("🧹 Nettoyage des ressources..."),y.cleanupDone=!0)}document.addEventListener("DOMContentLoaded",()=>{if(y.domReady){console.warn("⚠️  DOM déjà prêt");return}console.log("📄 DOM chargé"),y.domReady=!0,ye(),setTimeout(()=>{Ce()},100)});window.addEventListener("error",r=>{if(r.target!==window&&(r.target.tagName==="IMG"||r.target.tagName==="SCRIPT")){console.warn("⚠️  Ressource non chargée:",r.target.src||r.target.href);return}console.error("❌ Erreur:",r.message)},{once:!1,capture:!0});window.addEventListener("unhandledrejection",r=>{console.error("❌ Promesse rejetée:",r.reason),r.preventDefault()},{once:!1});window.addEventListener("beforeunload",Se,{once:!0});window.addEventListener("pageshow",r=>{(r.persisted||window.performance&&window.performance.navigation.type===2)&&(console.log("🔄 Page restaurée depuis le cache, rechargement..."),window.location.reload())});
