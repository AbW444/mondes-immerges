...
`),(t?t.querySelectorAll("button"):[]).forEach(o=>{o.style.cssText=`
    ...
    `,document.head.appendChild(t)}function Se(){if(y.initialized){console.warn("⚠️  Déjà initialisé");return}if(console.log("🌊 Initialisation Mondes Immergés..."),y.initialized=!0,!be())return;Ce();let r=0;const e=setInterval(()=>{r+=12.5,r>=100&&(clearInterval(e),console.log("✅ Chargement terminé"),setTimeout(()=>{we()},300))},200)}function Te(){y.cleanupDone||(console.log("🧹 Nettoyage des ressources..."),y.cleanupDone=!0)}document.addEventListener("DOMContentLoaded",()=>{if(y.domReady){console.warn("⚠️  DOM déjà prêt");return}console.log("📄 DOM chargé"),y.domReady=!0,xe(),setTimeout(()=>{Se()},100)});window.addEventListener("error",r=>{if(r.target!==window&&(r.target.tagName==="IMG"||r.target.tagName==="SCRIPT")){console.warn("⚠️  Ressource non chargée:",r.target.src||r.target.href);return}console.error("❌ Erreur:",r.message)},{once:!1,capture:!0});window.addEventListener("unhandledrejection",r=>{console.error("❌ Promesse rejetée:",r.reason),r.preventDefault()},{once:!1});window.addEventListener("beforeunload",Te,{once:!0});window.addEventListener("pageshow",r=>{(r.persisted||window.performance&&window.performance.navigation.type===2)&&(console.log("🔄 Page restaurée depuis le cache, rechargement..."),window.location.reload())});
