...
`),(t?t.querySelectorAll("button"):[]).forEach(o=>{o.style.cssText=`
    ...
    `,document.head.appendChild(t)}function Me(){if(f.initialized){console.warn("⚠️  Déjà initialisé");return}if(console.log("🌊 Initialisation Mondes Immergés..."),f.initialized=!0,!he())return;Ee(),fe();let r=0;const e=setInterval(()=>{r+=12.5,r>=100&&(clearInterval(e),console.log("✅ Chargement terminé"),setTimeout(()=>{ye()},300))},200)}function Ce(){f.cleanupDone||(console.log("🧹 Nettoyage des ressources..."),f.cleanupDone=!0)}document.addEventListener("DOMContentLoaded",()=>{if(f.domReady){console.warn("⚠️  DOM déjà prêt");return}console.log("📄 DOM chargé"),f.domReady=!0,be(),setTimeout(()=>{Me()},100)});window.addEventListener("error",r=>{if(r.target!==window&&(r.target.tagName==="IMG"||r.target.tagName==="SCRIPT")){console.warn("⚠️  Ressource non chargée:",r.target.src||r.target.href);return}console.error("❌ Erreur:",r.message)},{once:!1,capture:!0});window.addEventListener("unhandledrejection",r=>{console.error("❌ Promesse rejetée:",r.reason),r.preventDefault()},{once:!1});window.addEventListener("beforeunload",Ce,{once:!0});
