...
`),(t?t.querySelectorAll("button"):[]).forEach(i=>{i.style.cssText=`
        ...
        `;t.area&&(o+=`<strong>Superficie:</strong> ${t.area}<br>`),t.iceExtent&&(o+=`<strong>Étendue de glace:</strong> ${typeof t.iceExtent=="object"?`Hiver: ${t.iceExtent.winter}, Été: ${t.iceExtent.summer}`:t.iceExtent}<br>`),t.annualInput&&(o+=`<strong>Apport annuel:</strong> ${t.annualInput}<br>`),t.economicValue&&(o+=`<strong>Valeur économique:</strong> ${t.economicValue}<br>`),o+="</div>";let i="";e.detailedInfo&&(i=`
            ...
            ${i}
            ${s}
        `}handleHotspotExit(){this.contentPanel&&this.contentPanel.hide&&this.contentPanel.hide();const e=document.querySelector(".coordinates-display");e&&(e.style.backgroundColor="rgba(0, 0, 0, 0.7)",e.style.borderColor="rgba(255, 204, 0, 0.3)"),this.interfaceUI&&this.interfaceUI.setUIVisibility&&this.interfaceUI.setUIVisibility(!0),this.currentHotspot=null}}const j=new de;function ue(){j.init()}function pe(){return j}const b={initialized:!1,webGLChecked:!1,domReady:!1,appStarted:!1,cleanupDone:!1};function me(){if(b.webGLChecked)return!0;console.log("🔍 Vérification WebGL...");try{const r=document.createElement("canvas"),e=r.getContext("webgl")||r.getContext("experimental-webgl");if(!e)return console.error("❌ WebGL non supporté"),he(),b.webGLChecked=!1,!1;console.log("✅ WebGL compatible"),b.webGLChecked=!0;const t=e.getExtension("WEBGL_lose_context");return t&&t.loseContext(),!0}catch(r){return console.error("❌ Erreur vérification WebGL:",r),!1}}function he(){const r=document.getElementById("loading-screen");r&&(r.innerHTML=`
    ...
    `,document.head.appendChild(t)}function Ee(){if(b.initialized){console.warn("⚠️  Déjà initialisé");return}if(console.log("🌊 Initialisation Mondes Immergés..."),b.initialized=!0,!me())return;we();let r=0;const e=setInterval(()=>{r+=12.5,r>=100&&(clearInterval(e),console.log("✅ Chargement terminé"),setTimeout(()=>{be()},300))},200)}function Me(){b.cleanupDone||(console.log("🧹 Nettoyage des ressources..."),b.cleanupDone=!0)}document.addEventListener("DOMContentLoaded",()=>{if(b.domReady){console.warn("⚠️  DOM déjà prêt");return}console.log("📄 DOM chargé"),b.domReady=!0,ge(),setTimeout(()=>{Ee()},100)});window.addEventListener("error",r=>{if(r.target!==window&&(r.target.tagName==="IMG"||r.target.tagName==="SCRIPT")){console.warn("⚠️  Ressource non chargée:",r.target.src||r.target.href);return}console.error("❌ Erreur:",r.message)},{once:!1,capture:!0});window.addEventListener("unhandledrejection",r=>{console.error("❌ Promesse rejetée:",r.reason),r.preventDefault()},{once:!1});window.addEventListener("beforeunload",Me,{once:!0});window.addEventListener("pageshow",r=>{(r.persisted||window.performance&&window.performance.navigation.type===2)&&(console.log("🔄 Page restaurée depuis le cache, rechargement..."),window.location.reload())});
